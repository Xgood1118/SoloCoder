...
async function test5_loggerLevels() {
  const logger = new Logger({ level: 'debug', targets: ['console'] }, 'test-svc', 'test');
  let captured: any[] = [];
  const origLog = console.log;
  const origDebug = console.debug;
  const origWarn = console.warn;
  console.log = (...args: any[]) => captured.push(['info', ...args]);
  console.debug = (...args: any[]) => captured.push(['debug', ...args]);
  console.warn = (...args: any[]) => captured.push(['warn', ...args]);
...
async function test6_loggerFilter() {
  const logger = new Logger({ level: 'warn', targets: ['console'] }, 'test-svc', 'test');
  let captured: any[] = [];
  const origLog = console.log;
  const origDebug = console.debug;
  const origWarn = console.warn;
  console.log = (...args: any[]) => captured.push('info');
  console.debug = (...args: any[]) => captured.push('debug');
  console.warn = (...args: any[]) => captured.push('warn');
...
async function test8_errorCodes() {
  ...
  const missing = required.filter(c => !(c in ErrorCode));
...
async function test9_errorCategories() {
  const ok = errorCategories[ErrorCode.NETWORK_ERROR].retryable === true
    && errorCategories[ErrorCode.NOT_FOUND].retryable === false
    && errorCategories[ErrorCode.VALIDATION_ERROR].retryable === false
    && errorCategories[ErrorCode.RATE_LIMITED].retryable === true
    && errorCategories[ErrorCode.POOL_EXHAUSTED].retryable === true
    && errorCategories[ErrorCode.RETRY_TIMEOUT].retryable === false;
...
async function test10_fromHttpStatus() {
  ...
  for (const [status, expected] of cases) {
    const err = SdkError.fromHttpStatus(status, `test ${status}`);
...
async function test11_retryBackoff() {
  ...
  const inRange = (n: number, expected: number) => n >= expected * 0.9 && n <= expected * 1.1;
...
async function test13_retryShouldRetry() {
  ...
  const retryableError = SdkError.fromHttpStatus(500);
  const nonRetryableError = SdkError.fromHttpStatus(404);
  const nonRetryableConfig = SdkError.fromError(new Error('config error'));

  // attempt 1: should retry retryable
  // attempt 3: should NOT retry (>= maxRetries)
  const ok =
    rs.shouldRetry(retryableError, 1) === true
...
async function test14_retryExecute() {
  ...
  try {
    await rs.execute(async () => {
      attempts++;
      if (attempts < 3) {
        throw SdkError.fromHttpStatus(500);
      }
      return 'ok';
    });
  } catch (e: any) {
...
async function test15_retryExhausted() {
  ...
  const ok = attempts === 2 && (caught instanceof SdkError) && caught.code === ErrorCode.RETRY_TIMEOUT;
...
async function test16_multiInstance() {
  ...
  const ok = id1 !== id2
    ...
    && SdkCore.getAllInstances().length >= 2;
  await sdk1.close();
  await sdk2.close();
  record('multiInstance (independent)', ok, `id1!=id2=${id1 !== id2}, serviceName1=${cfg1.serviceName}, serviceName2=${cfg2.serviceName}, all instances count=${SdkCore.getAllInstances().length}`);
}
...
async function test17_notInitialized() {
  ...
  const ok = caught instanceof SdkError && caught.code === ErrorCode.CONFIG_ERROR;
...
async function test19_http404() {
  ...
  await sdk.close();
  const ok = caught instanceof SdkError && caught.code === ErrorCode.NOT_FOUND && caught.statusCode === 404;
...
async function test20_http500() {
  ...
  await sdk.close();
  const ok = caught instanceof SdkError && caught.code === ErrorCode.INTERNAL_SERVER_ERROR;
...
async function test21_globalTimeout() {
  ...
  await sdk.close();
  const ok = caught instanceof SdkError && (caught.code === ErrorCode.REQUEST_TIMEOUT || caught.code === ErrorCode.CONNECTION_TIMEOUT) && elapsed < 2000;
...
async function test24_connectionPool() {
  let created = 0;
  let destroyed = 0;
  const factory: ConnectionFactory<string> = {
...
async function test25_poolQueueing() {
  ...
  const pool = new ConnectionPool<string>(
    { minConnections: 1, maxConnections: 2, idleTimeoutMs: 60000, acquireTimeoutMs: 3000 },
    factory,
    'q-pool',
  );
  ...
  pool.release(c2);
  await pool.close();
  const ok = caught instanceof SdkError && caught.code === ErrorCode.POOL_TIMEOUT;
...
async function test29_closeReinit() {
  const sdk = await SdkCore.createInstance({ serviceName: 'reinit-test' });
  const id1 = sdk.getInstanceId();
  await sdk.close();
  const reinit = await SdkCore.createInstance({ serviceName: 'reinit-test-2' });
...
async function test30_getInstance() {
  const sdk = await SdkCore.createInstance({ serviceName: 'getinst-test' });
  const id = sdk.getInstanceId();
  const retrieved = SdkCore.getInstance(id);
