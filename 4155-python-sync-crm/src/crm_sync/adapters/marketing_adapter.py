...
class MarketingSource(SyncSource):
    ...
    def read_batch(self, batch_size: int, **kwargs: Any) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to read batch from Marketing {self.entity_type}: {e}")
            return []
    ...
    def get_count(self, **kwargs: Any) -> int:
        ...
        try:
            ...
            logger.error(f"Failed to get count from Marketing {self.entity_type}: {e}")
            return 0
    ...
    def read_by_id(self, record_id: str, **kwargs: Any) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to read record {record_id} from Marketing: {e}")
            return None
...
class MarketingTarget(SyncTarget):
    ...
    def write(self, data: Dict[str, Any], **kwargs: Any) -> Optional[str]:
        ...
        try:
            ...
            record_id = response.get("data", {}).get("uuid") or response.get("data", {}).get("id")
            if record_id:
                self._update_mapping(data, str(record_id))
            return str(record_id) if record_id else None
        except Exception as e:
            logger.error(f"Failed to write to Marketing {self.entity_type}: {e}")
            return None
    ...
    def update(self, record_id: str, data: Dict[str, Any], **kwargs: Any) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to update record {record_id} in Marketing: {e}")
            return False
    ...
    def delete(self, record_id: str, **kwargs: Any) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to delete record {record_id} from Marketing: {e}")
            return False
    ...
    def check_circular_sync(self, data: Dict[str, Any]) -> bool:
        ...
        return sync_source == "marketing"

    def _find_existing_record(
        self, data: Dict[str, Any], deduplication_fields: List[str]
    ) -> Optional[str]:
        local_id = data.get("sync_original_id")
        if local_id:
            mapping = (
                self.db.query(SyncMapping)
                .filter(
                    SyncMapping.local_id == str(local_id),
...
class MarketingVerifier(SyncVerifier):
    def __init__(self, entity_type: str, api_adapter: BaseAPIAdapter):
        super().__init__(entity_type)
        self.api = api_adapter
        self._endpoint_map = {
    ...
    def fetch_target_data(self, record_id: str, **kwargs: Any) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to fetch record {record_id} from Marketing: {e}")
            return None
...
class MarketingAdapter:
    def __init__(self, db_session: Optional[Session] = None):
        ...
        self.api = BaseAPIAdapter(
            base_url=settings.marketing.api_base_url,
            api_key=settings.marketing.api_key,
            timeout=settings.marketing.timeout,
        )
        self.db_session = db_session

    def get_source(self, entity_type: str) -> MarketingSource:
        return MarketingSource(entity_type, self.api)

    def get_target(self, entity_type: str, db_session: Optional[Session] = None) -> MarketingTarget:
        session = db_session or self.db_session
        if not session:
            raise ValueError("Database session is required for target adapter")
        return MarketingTarget(entity_type, self.api, session)

    def get_verifier(self, entity_type: str) -> MarketingVerifier:
        return MarketingVerifier(entity_type, self.api)

    def close(self) -> None:
        self.api.close()


class MarketingContactAdapter(MarketingAdapter):
    pass


class MarketingLeadAdapter(MarketingAdapter):
    pass


class MarketingOrderAdapter(MarketingAdapter):
    pass
