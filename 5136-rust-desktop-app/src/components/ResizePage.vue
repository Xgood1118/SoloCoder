    ...
    <div class="config-section">
      ...
      <div class="form-group">
        ...
        <select v-model="mode" class="form-select">
          <option value="by_width">按宽度等比缩放</option>
          <option value="by_height">按高度等比缩放</option>
          <option value="fixed">固定宽高</option>
        </select>
      </div>

      <div class="form-row">
        <div v-if="mode !== 'by_height'" class="form-group">
          <label>宽度 (px)</label>
          <input v-model.number="width" type="number" min="1" class="form-input" placeholder="宽度" />
        </div>
        <div v-if="mode !== 'by_width'" class="form-group">
          <label>高度 (px)</label>
          <input v-model.number="height" type="number" min="1" class="form-input" placeholder="高度" />
        </div>
      </div>
...
import { batchResize } from '../api'
import type { ResizeMode } from '../types'

const batch = useBatch()

const mode = ref<ResizeMode>('by_width')
...
const canStart = computed(() => {
  ...
  if (mode.value !== 'by_height' && (!width.value || width.value <= 0)) return false
  if (mode.value !== 'by_width' && (!height.value || height.value <= 0)) return false
  return true
})
...
async function startResize() {
  await batch.startProcess(() =>
    batchResize(batch.taskId, [...batch.fileList.value], {
      mode: mode.value,
      width: mode.value !== 'by_height' ? width.value : undefined,
      height: mode.value !== 'by_width' ? height.value : undefined,
